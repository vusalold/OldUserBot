...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "olduserjson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.olduserjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.old userjson", "r").read())
                else:
            ...
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.olduserjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.olduserjson", "r").read())
                else:
                    raise Exception("Your language file is invalid")
        break

if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.olduserjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.olduserjson", "r").read())
        except JSONDecodeError:
