...
strtobool = lambda x: True if x.lower() == 'true' else False
...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @OldUserBotSupports - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @OldUserBotSupports - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/vusalold/OldUserBot")

# Ayrıntılı konsol günlügü
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///olduser.db")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@OldUserBotSupports Paketi")
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@OldUserBotSupports"))
            bot(JoinChannelRequest("@OldUserBott"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @OldUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir OldUser açabilirsin; Kanala bak` @OldUserBot')
            else:
                await event.reply(f'`Tengri save Turks! OldUser working... 🐺`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@OldUserBott":
                rev_text = query[::-1]
                veriler = (butonlastir(0, CMD_HELP))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**🐺 Tanrı Türk'ü Korusun!** [OldUser](https://t.me/OldUserBott) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @OldUserBott",
                    text="""@OldUserBott'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/OldUserBott"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/OldUserBotSupports")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/vusalold/OldUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @OldUserBott kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**🐺 Tanrı Türk'ü Korusun!** [OldUser](https://t.me/OldUserBott) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
